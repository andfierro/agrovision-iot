...
// Función para simular lectura de sensores físicos (ESP32/Arduino)
const readIoTData = (): SensorData => ({
  temperature: 22 + Math.random() * 8, // Simula 22-30°C
  humidity: 50 + Math.random() * 40,   // Simula 50-90% humedad
  soilMoisture: 30 + Math.random() * 50, // Simula humedad de suelo
  lightLevel: 500 + Math.random() * 500, // Simula lúmenes
});
...
const App: React.FC = () => {
  ...
  // Estado para los datos de sensores (IoT)
  ...
  const runAnalysis = async (base64Image: string) => {
    ...
    // Usamos setTimeout para evitar que el renderizado se bloquee mientras TFJS carga
    setTimeout(async () => {
      ...
      <header className="bg-white px-6 pt-12 pb-4 shadow-sm z-10 sticky top-0">
        <div className="flex justify-between items-start">
          ...
          >
             ...
             <span>{checkingModel ? 'CONECTANDO...' : (modelReady ? 'GEMINI ACTIVO' : 'SIN API KEY')}</span>
          </button>
        </div>
      </header>
      ...
      <main className="flex-1 p-6 overflow-y-auto">
        ...
        {!image && (
          <div className="flex flex-col h-full animate-fade-in">
            {/* Pasamos los datos del estado principal al Dashboard */}
              ...
              <div className="bg-green-50 rounded-full p-8 border-2 border-dashed border-green-200 relative group">
                ...
                {!modelReady && !checkingModel && (
                   <div className="absolute -bottom-8 left-1/2 -translate-x-1/2 w-56 bg-red-100 text-red-800 text-[10px] py-2 px-3 rounded-lg border border-red-200 text-center shadow-sm z-20">
                     ...
                     Falta <code>API_KEY</code> en variables de entorno.
                   </div>
                )}
              </div>
              <div className="text-center">
                <h2 className="text-xl font-bold text-gray-800">Agro-Diagnóstico AI</h2>
                <p className="text-gray-500 mt-2 px-4 text-sm">
                  Análisis avanzado con Gemini 2.5 Flash + Fusión de Sensores.
                </p>
              </div>
            </div>
        ...
        {image && isAnalyzing && (
          <div className="flex flex-col items-center justify-center h-full space-y-6 animate-pulse">
            ...
            <div className="text-center">
              <h3 className="text-xl font-bold text-gray-800">Analizando Cultivo</h3>
              <p className="text-sm text-gray-500 mt-1">
                Consultando modelo Gemini y procesando datos...
              </p>
            </div>
          </div>
        )}
